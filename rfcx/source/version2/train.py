...
def reduceMetric(label_array, pred_array):
    ...
    score = (label_array == pred_array).astype(float).mean()
    score = round(score, 4)
    return score
...
def validModel(model, data, loss_fn):
    ...
    with torch.no_grad():
        for sample in data:
            ...
            label = Variable(sample[1].float().squeeze().to(DEVICE))
...
def trainModel(model, train_data, valid_data, loss_fn, optimizer, save_path, epochs, batch, scheduler):
    ...
    update_bn(train_data, swa_model, 'cuda:0')
    saveModel(epoch, swa_model, valid_metric, save_path.replace('.pt','_swa.pt'))
    logfile.close()
    return None