...
class TrainDataset(Dataset):
    ...
    def __fetchitem__(self, idx):
        ...
        label_ = [0] * 24
        label_[label] = 1.
        label = np.array(label_).astype(int)
    ...
    def __getitem__(self, idx):
        ...
        image1, label1 = self.__fetchitem__(idx1)
        image2, label2 = self.__fetchitem__(idx2)
        image = lam * image1 + (1. - lam) * image2
        label = lam * label1 + (1. - lam) * label2
        image = torch.tensor(image, dtype=torch.float)
        label = torch.tensor(label, dtype=torch.float).reshape(24,)
        sample = (image, label)
        return sample
...
class ValidDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        image = self.__image__(audio, tmin, tmax)
        image = torch.tensor(image, dtype=torch.float)
        label = torch.tensor(label_, dtype=torch.float).reshape(24,)
        sample = (image, label)
        return sample
