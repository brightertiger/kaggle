...
def trainTF():
    transforms = []
    transforms += [A.PadIfNeeded(600,800, p=1.0)]
    transforms += [A.Resize(600, 800, p=1.0)]
    transforms += [A.RandomResizedCrop(512, 512, p=1.0)]
...
def validTF():
    ...
    transforms += [A.PadIfNeeded(600,800, p=1.0)]
    transforms += [A.Resize(600, 800, p=1.0)]
    transforms += [A.CenterCrop(512, 512, p=1.)]
...
class ValidDataset(Dataset):
    ...
    def __init__(self, path, data, fold):
        ...
        self.data = data[(data['fold'] == fold) & (data['source'] == 2020)]
        self.data = self.data.reset_index(drop=True)
        return None
