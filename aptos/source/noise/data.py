...
class ImageDataset(Dataset):
    ...
    def __getitem__(self, idx):
        image_1 = self.__image__(self.image[idx])
        image_2 = self.__image__(self.image[idx])
        label = self.__label__(idx)
        weight = torch.from_numpy(np.array([self.weight])).reshape(-1,1)
        sample = {'idx': self.image[idx], 'image_1': image_1, 'image_2': image_2, 'label': label, 'weight': weight}
        return sample
